...
if [[ -d "$SPECS_DIR" ]]; then
    ...
    if [[ -n "$HIGHEST" && "$HIGHEST" != "000" ]]; then
        # Increment, handling leading zeros
        NEXT_NUM=$((10#$HIGHEST + 1))
        NEXT_ID=$(printf "%03d" "$NEXT_NUM")
    fi
    
    # Build list of existing features (simple JSON array)
    FEATURES_LIST=""
...
# Intake directory (separate from specs/)
...
EOF
    ...
    echo "CONSTITUTION_PATH: $CONSTITUTION_FILE"
fi
